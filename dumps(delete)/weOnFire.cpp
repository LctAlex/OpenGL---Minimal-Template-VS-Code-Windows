...
//EVERYTHING needs to go inside a class (ex. GLFWwindow window, float deltaTime(time between frames) as members)
...
void CircleData(std::vector<float>& vertices, std::vector<unsigned int>& indices)
{
    ...
    for(int i = 0; i < sectors; i++)
    {
        ...
        float xAxis = radius * cos(angle*DEG2RAD); //cos and sin expect RADIANS...
        float yAxis = radius * sin(angle*DEG2RAD);
...
unsigned int HardcodedShader()
{
    ...
    "   FragColor = vec4(PosColor.x+sin(time/2.0)+0.5, PosColor.y+cos(time/2.0)+0.5, 0.0f, 1.0f);\n"
    "}\0";
